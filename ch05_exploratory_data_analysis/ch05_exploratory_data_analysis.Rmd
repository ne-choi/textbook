---
title: "05. Exploratory Data Analysis"
author: "ne_choi"
date: '2020 10 5 '
output:
  html_document:
...
**준비하기**
이 장에서는 dplyr과 ggplot2에 대해 배운 내용을 질문하기, 데이터로 답하기 및 새롭게 질문하는 과정과 결합할 것이다.<br><br></font>
...
<ul>
<li type = "disc">
연속형(continuous) 변수는 순서가 있는 무한 집합에서 임의의 값을 가질 수 있는 변수
<li type = "disc">
...
* 정보를 유용한 질문으로 전환하기 위해 예상하지 못한 부분 찾기  
- 어떤 값이 가장 일반적인가?  
- 드물에 나타나는 값은 무엇인가?  
- 비정상적인 패턴을 볼 수 있는가? 패턴을 설명하라.  

ex) 아래 히스토그램은 몇 가지 흥미로운 질문을 제시한다.
```{r}
ggplot(data = smaller, mapping = aes(x = carat)) +
  geom_histogram(binwidth = 0.01)
```
- 전체 캐럿 중 일부 캐럿에 해당하는 다이아몬드가 많은 이유는?  
- 각 최고점의 오른쪽 부분에 왼쪽 부분보다 많은 다이아몬드가 있는 이유는?  
- 3캐럿보다 큰 다이아몬드가 없는 이유는?  
