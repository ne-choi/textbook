...
R 마크다운으로 생성할 수 있는 더 많은 출력 유형 알아보기  
(현재까지 공부한 R 마크다운은 HTML 문서를 생성하는 방법)  
...
```
rmarkdown::remder("diamond-size.Rmd", output_format = "word_document")  
```
...
그 뒤, `입력` 함수를 이용하여 문서에 대화형 구성요소 추가 가능 
...
textInput("name", "What's your name?") # 한글이 깨져서 영문으로 입력
